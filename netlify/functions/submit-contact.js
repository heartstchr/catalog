...
export default async function handler(event, context) {
    ...
    // Use contact database ID if available, otherwise fall back to default
    const notionDatabaseId = process.env.NOTION_CONTACT_DATABASE_ID || process.env.NOTION_DATABASE_ID;
    ...
    if (!notionApiKey || !notionDatabaseId) {
        console.error('Missing Notion configuration:', {
            ...
            hasContactDatabaseId: !!process.env.NOTION_CONTACT_DATABASE_ID,
            hasDatabaseId: !!process.env.NOTION_DATABASE_ID,
            resolvedDatabaseId: !!notionDatabaseId
        });
        return new Response(
            JSON.stringify({
                error: "Notion configuration missing. Please set NOTION_API_KEY and NOTION_CONTACT_DATABASE_ID (or NOTION_DATABASE_ID) environment variables."
            }),
            { status: 500, headers }
        );
    }
            ...
            if (value !== null && value !== undefined && value !== '') {
                switch (key) {
                    ...
                    case 'website':
                        ...
                        break;
                    case 'linkedin':
                        notionProperties["LinkedIn"] = { url: value };
                        break;
                    case 'budget':
